...
func main() {
	body := davudpasha.SourcesSearchRequest{
		SearchFilter:            common.PtrString("test-source"),
	...
	resp, r, err := api.SearchSources(ctx, *davudpasha.NewSearchSourcesOptionalParameters().WithBody(body))
	...
	fmt.Fprintf(os.Stdout, "Response from `SourcesApi.SearchSources`:\n%s\n", responseContent)
